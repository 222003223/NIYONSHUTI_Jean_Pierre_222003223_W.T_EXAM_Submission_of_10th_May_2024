...
<head>
    ...
    <title>AR attendees</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
     <link rel="stylesheet" href="css/style5.css">
</head>
<body>    <header> 
        <h1><img src="img/ig1.png" alt="icon"> AR attendees</h1>
</header>
<header>
      <nav>
        <ul>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ar attendee Management</title>
    ...
    <table id="dataTable" class="table table-hover text-center">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row["attendee_id"] . "</td>"; // attendee_id
                echo "<td>" . $row["workshop_id"] . "</td>"; // workshop_id
                echo "<td>" . $row["user_id"] . "</td>"; // user_id
                echo "<td>" . $row["registration_date"] . "</td>"; // registration_date
                echo "<td>";
            }
        } else {
