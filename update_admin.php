...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if ($rowsAffected > 0) {
            echo "<script>alert('admin data updated successfully!'); window.location.href = 'admin_home.html';</script>";
        } else {
            echo "<script>alert('No changes were made.'); window.location.href = 'admin_home.html';</script>";
        }

    } catch(Exception $e) {
        echo "Error updating admin data: " . $e->getMessage();
        error_log("Update admin data error: " . $e->getMessage() . "\n", 3, "/path/to/error.log");
    }
}
?>
